...
/*   Updated: 2025/05/01 20:02:45 by fben-ham         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void					close_and_free(int *pipefd, char **cmd1, char **cmd2,
							int temp_fd);
int						count_args(char **args, int start);
void					free_commands(char **cmd1, char **cmd2,
							char *temp_file);
...
int						find_last_redirection(char **args,
							int *redirection_type);
int						execute_heredoc_pipe(char **args, t_shell *shell);
t_shell					*init_shell(char **envp);
void					free_env(char **env);
void					find_special_tokens(char **args, int *heredoc_idx,
							int *pipe_idx);
int						init_heredoc_pipe(t_hd_pipe *hp, char **args,
							int heredoc_idx, int pipe_idx);
int						setup_second_command(t_hd_pipe *hp, char **args,
							int pipe_idx);
int						count_pipes(char **args);
char					***split_command_by_pipes(char **args);
int						execute_simple_command(char **args, char **env_copy);
int						execute_piped_commands(char ***cmd_segments,
							char **env_copy);
int						execute_piped_commands_part1(char ***cmd_segments,
							t_shell *shell);
int						execute_piped_commands_part2(char ***cmd_segments,
							char **env_copy, int pipe_count);
...
void					execute_first_command(char **cmd1, int temp_fd,
							int *pipefd, t_shell *shell);
void					execute_second_command(char **cmd2, int *pipefd,
							int temp_fd, t_shell *shell);
int						count_pipes(char **args);
int						wait_for_children(int pipe_count);
int						execute_pipe_without_heredoc(char **args,
							t_shell *shell);
int						check_for_heredoc_pipe(char **args, int *has_heredoc,
							int *has_pipe);
int						init_command_pipes(int i, int pipe_count,
							int pipe_fds[2][2], int current_pipe);
int						execute_piped_commands_setup(char ***cmd_segments,
							t_shell *shell, int pipe_count, pid_t *pids);
int						count_segment_size(char **args, int i);
char					**create_segment(char **args, int *index, int seg_size);
int						wait_for_children(int pipe_count);
void					handle_parent_pipes(int i, int pipe_count,
							int pipe_fds[2][2], int *current_pipe);
void					setup_child_pipes(int i, int pipe_count,
							int pipe_fds[2][2], int current_pipe);
...
int						parse_redirection(char **args,
...
char					**tokenize_command_line(const char *line,
							t_shell *shell);
char					*get_env_value(char **envp, const char *var_name);
int						set_env_var(t_shell *shell, const char *varname,
							const char *value);
...
int						setup_heredoc_pipe(int pipefd[2], int *saved_stdin,
							pid_t *pid);
void					handle_heredoc_child(int pipefd[2], char *delimiter);
int						setup_redirection(char **args, t_redir_info *redir_info,
							t_shell *shell);
int						handle_heredoc_in_pipe(char **args, t_shell *shell);
...
int						handle_redirections(char **args,
							t_redir_info *redir_info, t_shell *shell);
void					restore_redirection(t_redir_info redir_info,
							int saved_fd, t_shell *shell);

// Pipe related functions (from pipes.c)
char					**create_pipe_segment(char **args, int *start_idx,
							int seg_size);
char					***split_command_by_pipes(char **args);
void					setup_child_pipes(int i, int pipe_count,
							int pipe_fds[2][2], int current_pipe);
...
// Pipe + Heredoc related functions (from pipes_heredoc.c)
int						check_for_heredoc_pipe(char **args, int *has_heredoc,
							int *has_pipe);
int						handle_heredoc_processing_for_pipe_segment(char **args,
							t_shell *shell);
// char	**copy_args_segment(char **args, int end_index); // Made static in .c
